...
import { deleteFile, saveFile } from "../../utils/file.js";
import {
  generateAccessToken,
...
export const setProfile = async (data) => {
  ...
  if (data.profileImage === null) {
    ...
    if (user.profile !== null) deleteFile(user.profile, "PROFILE");
    data.profileImageName = null;
  } else if (data.profileImage) {
    // 프로필 이미지가 존재한다면 업로드
    if (user.profile !== null) deleteFile(user.profile, "PROFILE");
    data.profileImageName = saveFile(data.profileImage, "PROFILE");
  } else {
    // undefined 일때 이미지를 변경하지 않음
    data.profileImageName = user.profile;
  }
