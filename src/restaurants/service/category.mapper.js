...
/** 네이버 cat/name → DB enum 문자열(FoodCategory) */
export function toFoodCategory(cat = "", name = "") {
  ...
  // 기타 케이스
  if (hasAny(cat, SEAFOOD) || hasAny(name, SEAFOOD)) return "KOREAN"; // 국내 분류에선 해산물 다수 한식으로 분류
  if (hasAny(cat, ASIAN) || hasAny(name, ASIAN)) return "WESTERN"; // 별도 enum 없으니 WESTERN/ETC 중 택1
